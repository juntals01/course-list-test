...
import { z } from 'zod';
import { ArrowLeft, Plus, Trash2, ChevronUp, ChevronDown, Info, Upload } from 'lucide-react';
...
export default function AddArticlePage() {
  const [sections, setSections] = useState<SectionItem[]>([
    { id: 1, title: '', description: '', expanded: true },
  ]);
  ...
  const {
    ...
    defaultValues: {
      ...
      articleName: '',
      mainCategory: '',
      sections: [{ id: 1, title: '', description: '' }],
    },
  });
  ...
  const onSaveAndClose = () => {
    handleSubmit((data) => {
      console.log('Article saved & closed:', data);
    })();
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex-1 px-4 md:px-6 py-4 flex flex-col">
      <Link
        ...
        className="inline-flex items-center gap-1.5 text-[var(--policiesAndProcedures-primary)] text-sm font-medium hover:underline mb-4 w-fit"
      >
        <ArrowLeft size={16} />
        Back
      </Link>
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">Create and publish articles</p>
      </div>
